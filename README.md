...
- **Guard:** scan or upload student ID-card images, optionally compare a live face, and record attendance.
...
Requirements: Python 3.11, Node 20+, a webcam for live capture, and roughly 2 GB free for the CV packages and model pack. The commands below use a standard project-local Python virtual environment; Conda is not required.
...
Copy `backend/.env.example`. Set `DATABASE_URL`, a long random `JWT_SECRET`, both face thresholds, `INSIGHTFACE_MODEL_ROOT`, `YOLO_MODEL_PATH`, `UPLOAD_DIRECTORY`, `ALLOW_MANUAL_ID_FALLBACK`, and allowed frontend origins. Secrets, model downloads, and biometric uploads are git-ignored.
